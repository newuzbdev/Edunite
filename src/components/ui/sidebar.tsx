...
function SidebarMenuSubButton({
  ...
  return (
    <Comp
      ...
      data-active={isActive}
      className={cn(
        "flex min-w-0 items-center gap-2 overflow-hidden rounded-md outline-hidden focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
        "transition-colors",
        size === "sm" && "text-xs h-7 px-2",
        size === "md" && "text-sm h-9 px-3",
        "group-data-[collapsible=icon]:hidden",
        className
      )}
      {...props}
